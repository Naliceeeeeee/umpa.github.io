...
(function () {
  ...
  function isExplicitResource(item) {
    ...
    if (cat === 'guides' || cat === 'pdf' || cat === 'clubs-schools' || cat === 'family-nfp' || cat === 'all') return true;
  ...
  function getResourcesFromStore() {
    ...
    Object.keys(store).forEach((type) => {
      const itemsOfType = store[type] || [];
      itemsOfType.forEach((item) => {
        const itemWithType = { ...item, type };
        if (isExplicitResource(itemWithType)) {
          if (!allResources.some((r) => r.id === item.id)) {
